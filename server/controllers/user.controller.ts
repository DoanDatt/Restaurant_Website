...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    const verificationToken = "ifneinfinieifie";
...
export const verifyEmail = async (res: Response, req: Request) => {
  try {
    ...
    // tìm user có mã xác minh trùng với verificationCode mà người dùng gửi lên
    const user = await User.findOne({
      // so sanh ma xac thuc cua nguoi dung gui len voi trong csdl
    ...
    await user.save();
    // await sendEmailWelcome({user.email, user.password})
    return res.status(200).json({
      success: true,
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const resetPasswordTokenExpiresAt = resetTokenExpiresAt;
    await user.save;
    // send email
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findOne({
      ...
      resetPasswordTokenExpiresAt: newPassword,
    });
    if (!user) {
      return res.status(400).json({
        success: false,
