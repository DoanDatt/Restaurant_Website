        ...
        <div className='flex-1'>
          ...
          <div>
            <div className='flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3'>
              <h1 className='font-medium text-lg'>(2) Search result food</h1>
              <div className='flex flex-wrap gap-2 mb-4 md:mb-0'>
                {['biryani', 'momos', 'jalebi'].map((selectedFilter: string, idx: number) => (
                  <div key={idx} className='relative inline-flex items-center max-w-full'>
                    <Badge className='bg-gray-200 text-[#D19254] rounded-md hover:cursor-pointer pr-6 whitespace-nowrap'>
                      {selectedFilter}
                    </Badge>
                  ...
                  <CardContent className='p-4'>
                    ...
                    <div className='flex gap-2 mt-4 flex-wrap'>
                      {['biryani', 'momos', 'jalebi'].map((cuisine: string, idx: number) => (
                        <Badge
                          key={idx}
                          className='bg-gray-200 text-gray-800 rounded-md hover:cursor-pointer px-3 py-1'
                        >
                          {cuisine}
                        </Badge>
                      ))}
...
const NoResultFound = ({ searchText }: { searchText: string }) => {
  return (
    <div className='text-center'>
      <h1 className='text-2xl font-semibold text-gray-700 dark:text-gray-200'>No results found</h1>
